...
while getopts "a:d:f:hl:s:" opt; do
  case $opt in
...
pushd $PROJECT_DIR
caffeinate -is pipenv run python src/fantasy_manager/cli/__init__.py roster replace claim --league $LEAGUE --add $ADD_ID --drop $DROP_ID $FAAB_OPT $START_OPT
exit_code=$?
popd

...
def mock_get_start_fixed(monkeypatch):
    monkeypatch.setattr(fantasy_manager.controller.roster, "get_start", lambda _: START_DT)


@pytest.fixture
def mock_get_start_midnight(monkeypatch):
    monkeypatch.setattr(fantasy_manager.controller.roster, "get_start", lambda _: NEXT_MIDNIGHT)
...
def test_replace_player_claim_no_start(controller, mock_service, mock_get_start_midnight):
